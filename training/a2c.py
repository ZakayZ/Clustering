    ...
    *,
    ...
    try:
        for _ in range(max(1, int(steps))):
            ...
            for _ in range(max(1, int(episodes_per_step))):
                ...
                with torch.no_grad():
                    ...
                    obs = ep["obs"]
                _, value_pred = ac(obs)
                return_tensor = torch.tensor(episode_return, dtype=value_pred.dtype)
                value_loss = 0.5 * (value_pred - return_tensor).pow(2)
                value_loss.backward()
...
def train_actor_critic(
    ...
    *,
    ...
    for u in pbar:
        ...
        raw_advantages = [
            episode_returns[i] - rollout_values[i] for i in range(len(batch_observations))
        ]
        if center_adv and raw_advantages:
            batch_advantage_mean = float(np.mean(raw_advantages))
            centered_advantages = [
                float(adv - batch_advantage_mean) for adv in raw_advantages
            ]
        else:
            centered_advantages = [float(adv) for adv in raw_advantages]

        ac.train()
        optimizer.zero_grad()
        policy_loss_sum = entropy_sum = value_loss_sum = 0.0
        for i in range(len(batch_observations)):
            obs = batch_observations[i]
            logits, value_pred = ac(obs, detach_value_features=bool(detach_value_features))
            ...
            advantage_tensor = torch.tensor(centered_advantages[i], dtype=logits.dtype)
            return_tensor = torch.tensor(episode_return, dtype=logits.dtype)
            policy_loss_term = -(mean_log_prob * advantage_tensor)
            value_loss_term = 0.5 * (value_pred - return_tensor).pow(2)
            combined_loss = (
                policy_coef * policy_loss_term
                + value_coef * value_loss_term
                - ent_coef * mean_entropy
            ) / batch_size
            combined_loss.backward()
            policy_loss_sum += policy_loss_term.item()
            entropy_sum += mean_entropy.item()
            value_loss_sum += value_loss_term.item()
        ...
        history["value_loss"].append(value_loss_sum / batch_size)
        history["value_mean"].append(
            float(np.mean(rollout_values)) if rollout_values else 0.0
        )
        history["advantage_mean"].append(
            float(np.mean(centered_advantages)) if centered_advantages else 0.0
        )
        ...
        progress_metrics["value_loss"] = float(value_loss_sum / batch_size)
